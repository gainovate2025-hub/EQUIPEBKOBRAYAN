...
const COLUMNS = [
  ...
  { key: 'done', label: 'Realizado', width: '.9fr', align: 'right' },
  { key: 'progress', label: 'Percentual', width: '1.4fr' },
]
...
export default function Contestacoes() {
  ...
  const rows = useMemo(
    () => team.map((b) => ({
      ...
      id: b.id,
      name: b.name,
      goal: b.performance?.contestation_goal ?? 0,
      done: b.performance?.contestations_done ?? 0,
    })),
    [team]
  )
      ...
      <div className="mt-9 flex flex-col gap-4">
        ...
        {loading ? (
          ...
          <DataTable
            columns={COLUMNS}
            rows={rows}
            totalRow={{ name: 'Total da equipe', goal: totalGoal, done: totalDone, progress: `${totalPct}%` }}
            renderCell={(row, key) => {
              if (key === 'name') return <span className="font-medium">{row.name}</span>
              if (key === 'goal') return <InlineEditableNumber value={row.goal} onCommit={(v) => commit(row.id, 'contestation_goal', v)} />
              if (key === 'done') return <InlineEditableNumber value={row.done} onCommit={(v) => commit(row.id, 'contestations_done', v)} />
              if (key === 'progress') {
